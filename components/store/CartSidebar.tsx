      ...
      {open && (
        <>
          ...
          <motion.aside
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-[var(--card)] z-[90] shadow-2xl flex flex-col"
          >
            <div className="p-5 border-b border-[var(--border)] flex items-center justify-between">
              <h3 className="font-black tracking-tight text-lg">YOUR BAG</h3>
              <button onClick={() => setOpen(false)} className="p-2 rounded-xl hover:bg-[var(--input-bg)]">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <div key={`${item.productId}-${item.size}`} className="border border-[var(--border)] rounded-xl p-3">
