<template>
  <div class="relative w-full rounded-[2rem] bg-gradient-to-br from-[#1a1010] to-[#050505] border border-red-500/20 overflow-hidden group">
    ...
    <!-- Glow Backdrop -->
    <div class="absolute inset-0 bg-red-500/5 group-hover:bg-red-500/10 transition-colors z-0 pointer-events-none"></div>
    <div class="absolute -top-32 -right-32 w-96 h-96 bg-red-500/20 blur-[100px] rounded-full z-0 pointer-events-none"></div>

    <div class="relative z-10 flex flex-col items-center justify-between gap-12 p-10 md:p-16">
      
      <div class="flex flex-col md:flex-row items-center w-full justify-between gap-12">
        <!-- Text content -->
        <div class="text-center md:text-left flex-1 max-w-xl">
          ...
          <router-link to="/products?badge=SALE" class="inline-block px-10 py-5 bg-gradient-to-r from-red-600 to-orange-500 text-white font-bold rounded-full tracking-widest uppercase transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(220,38,38,0.5)]">
            SHOP THE SALE <span class="text-xl ml-2">→</span>
          </router-link>
        </div>
      ...
      <!-- Flash Sale Products -->
      <div class="w-full mt-8 border-t border-red-500/20 pt-12">
        <h3 class="text-2xl font-display font-black tracking-widest text-white mb-8 border-l-4 border-red-500 pl-4">LOCKED DEALS</h3>
...
const fetchFlashSaleProducts = async () => {
  try {
    const response = await api.get('/products?badge=SALE&per_page=4')
    if (response.data.products && response.data.products.length > 0) {
      products.value = response.data.products
    } else {
      const fallback = await api.get('/products?featured=true&per_page=4')
      products.value = fallback.data.products
    }
  } catch (error) {
